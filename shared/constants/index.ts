...
const createPendingMock = (
  ...
  target_platforms: [platform, SourcePlatform.OTHER],
  original_source_url: '',
...
const createPublishedMock = (
  ...
  if (Math.random() > 0.7) {
    ...
    targetPlatforms.push(others[Math.floor(Math.random() * others.length)]);
  }
...
export const INITIAL_CONTENT: ContentItem[] = [
  ...
  createPendingMock(
    'P-102',
    'Yoga buổi sáng tại Bali',
    SourcePlatform.VOTEME,
    'Thể thao',
    'wellness_jane'
  ),
  createPendingMock(
  ...
  createPendingMock(
    'P-104',
    'Phân tích sụp đổ thị trường Crypto',
